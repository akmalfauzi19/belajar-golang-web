...
func TemplateAutoEscapeDisable(writer http.ResponseWriter, request *http.Request) {
	myTemplates.ExecuteTemplate(writer, "post.gohtml", map[string]interface{}{
		"Title": "Template Auto Escape",
		"Body":  template.HTML("<p>Ini Adalah Body<script>alert('Anda di Heck')</script></p>"),
	})

	// if use for css using template.CSS
}
...
func TestTemplateXSS(t *testing.T) {
	request := httptest.NewRequest(http.MethodGet, "http://localhost:8080/?body=<p>Iniww</p>", nil)
	recorder := httptest.NewRecorder()
